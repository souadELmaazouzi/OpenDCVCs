...
class DMC(CompressionModel):
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        with torch.no_grad():
            # Move tensor to CPU before using np.log
            mv_y_q_scale_enc_fine = np.linspace(np.log(self.mv_y_q_scale_enc[0, 0, 0, 0].cpu()),
                                            np.log(self.mv_y_q_scale_enc[3, 0, 0, 0].cpu()), 64)
            self.mv_y_q_scale_enc_fine = np.exp(mv_y_q_scale_enc_fine)
            mv_y_q_scale_dec_fine = np.linspace(np.log(self.mv_y_q_scale_dec[0, 0, 0, 0].cpu()),
                                            np.log(self.mv_y_q_scale_dec[3, 0, 0, 0].cpu()), 64)
            self.mv_y_q_scale_dec_fine = np.exp(mv_y_q_scale_dec_fine)

            y_q_scale_enc_fine = np.linspace(np.log(self.y_q_scale_enc[0, 0, 0, 0].cpu()),
                                            np.log(self.y_q_scale_enc[3, 0, 0, 0].cpu()), 64)
            self.y_q_scale_enc_fine = np.exp(y_q_scale_enc_fine)
            y_q_scale_dec_fine = np.linspace(np.log(self.y_q_scale_dec[0, 0, 0, 0].cpu()),
                                            np.log(self.y_q_scale_dec[3, 0, 0, 0].cpu()), 64)
    ...
    def forward_one_frame(self, x, dpb, q_in_ckpt=False, q_index=None, frame_idx=0,stage=1,lmbda=0.01):
        ...
        scales_hat = 5.55 - torch.nn.functional.softplus(5.55 - scales_hat)  # make logscale < 5.55
        scales_hat = torch.exp(scales_hat)
        mv_scales_hat = torch.nn.functional.softplus(mv_scales_hat+2.3)-2.3 #make logscale > -2.3
        mv_scales_hat = torch.exp(mv_scales_hat)
    ...
    def forward(self, x, dpb, q_in_ckpt=False, q_index=None, frame_idx=0,stage=1,lmbda=0.01):
        return self.forward_one_frame(x, dpb, q_in_ckpt=q_in_ckpt, q_index=q_index,
