...
def train_one_epoch_fully_batched(model, i_frame_model, train_loader, optimizer, device, stage, epoch, gradient_accumulation_steps=1, finetune=False, grad_clip_max_norm=None, ema=None,args=None):
    ...
    if stage in [2, 3]:  # Freeze MV generation part in stages 2 and 3
        ...
        for param in model.mv_y_spatial_prior.parameters():
            param.requires_grad = False
        model.mv_y_q_basic.requires_grad = False
        model.mv_y_q_scale.requires_grad = False
        ...
        for param in model.mv_y_spatial_prior.parameters():
            param.requires_grad = True
        model.mv_y_q_basic.requires_grad = True
        model.mv_y_q_scale.requires_grad = True
        ...
        if finetune and stage == 4:
            ...
            for frame_pos in range(seq_length):
                ...
                if frame_pos == 0:  # First frame (I-frame) in each sequence
                    ...
                    result = model(current_frames, dpb, stage=stage,lmbda =lmbda)
...
def evaluate_fully_batched(model, i_frame_model, test_loader, device, stage, finetune=False,args=None):
    ...
    i_frame_scales= i_frame_model.q_scale
    p_frame_mv_scales = model.mv_y_q_scale
    p_frame_scales = model.y_q_scale
    lmbda_set = [85, 170, 380, 840]
    Total_results = OrderedDict()
    for quality_index in range(len(i_frame_scales)): 
            ...
            for batch_frames in test_loader:
                ...
                for frame_pos in range(seq_length):
                    ...
                    if frame_pos == 0:  # I-frames
                        ...
                        i_frame_results = i_frame_model.encode_decode(current_frames,i_frame_scales[quality_index])
                        ...
                        # Process all P-frames in the batch with their corresponding reference frames
                        result = model(current_frames, dpb, stage=stage,mv_y_q_scale=p_frame_mv_scales[quality_index],y_q_scale=p_frame_scales[quality_index],lmbda =lmbda_set[quality_index])
    ...
    Total_results["loss"]  = sum([Total_results[i]["loss"] for i in range(len(i_frame_scales))]) / len(i_frame_scales)
            ...
            for batch_frames in test_loader:
                ...
                for frame_pos in range(seq_length):
                    if frame_pos == 0:
                        ...
                        i_frame_results = i_frame_model.encode_decode(previous_frames,i_frame_scales[quality_index])
                        reference_frames = i_frame_results["x_hat"]  # Shape: [B, C, H, W]
                        dpb = {
                            "ref_frame": reference_frames,
                            "ref_feature": None,
                        ...
                        # Process all P-frames in the batch with their corresponding reference frames
                        IP_result = model(current_frames, dpb, stage=stage,mv_y_q_scale=p_frame_mv_scales[quality_index],y_q_scale=p_frame_scales[quality_index],lmbda =lmbda_set[quality_index])
                        ...
                        if stage==1:
                            dpb = {
                                "ref_frame": IP_result["pixel_rec"],
                                "ref_feature": None,
                                "ref_y": None,
                                "ref_mv_y": IP_result["ref_mv_y"] if not args.warmup else None,
                            }
                        else:
                            if args.warmup:
                                dpb = {
                                    "ref_frame": IP_result["dpb"]["ref_frame"],
                                    "ref_feature": None,
                                    "ref_y": None,
                                    "ref_mv_y": None,
                                }
                        PP_result = model(next_frames, dpb, stage=stage,mv_y_q_scale=p_frame_mv_scales[quality_index],y_q_scale=p_frame_scales[quality_index],lmbda =lmbda_set[quality_index])
...
def main():
    ...
    parser.add_argument('--global_best_dir', type=str, default='best_ckpt_transform',
    ...
    if args.spynet_from_dcvc_checkpoint:
        ...
        mv_y_q_basic = spynet_checkpoint['mv_y_q_basic']
        mv_y_q_scale = spynet_checkpoint['mv_y_q_scale']
        
        unwrap_model(model).optic_flow.load_state_dict(spynet_state_dict, strict=True)
        unwrap_model(model).mv_y_q_basic.data.copy_(mv_y_q_basic)
        unwrap_model(model).mv_y_q_scale.data.copy_(mv_y_q_scale)
    ...
    # Load from previous stage checkpoint if no resume but previous_stage_checkpoint is specified
        ...
        try:
            ...
            if args.not_skip_test: 
                ...
                ori_wamrup = args.warmup 
                args.warmup = False
                test_stats_three = evaluate_fully_batched_three(
                    model, i_frame_model, test_loader, device, args.stage-1,args
                )
                args.warmup = ori_wamrup
                # Log results
                with open(log_file, 'a') as f:
                    f.write(f"From stage {args.stage-1}:\n")
                    f.write(f"  Test Loss Overall (Normal): {test_stats_three['loss']:.6f}\n")
                    for q_index in range(len(model.mv_y_q_scale)):
                        f.write(f"  Test Loss Quality {q_index}: {test_stats_three[q_index]['loss']:.6f}\n")
                        f.write(f"  Test MSE Quality {q_index}: {test_stats_three[q_index]['mse']:.6f}\n")
                        f.write(f"  Test PSNR Quality {q_index}: {test_stats_three[q_index]['psnr']:.4f}\n")
                        f.write(f"  Test BPP Quality {q_index}: {test_stats_three[q_index]['bpp']:.6f}\n")
        except Exception as e:
