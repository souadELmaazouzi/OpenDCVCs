...
def precompute_reference_frames(dataset, i_frame_model, device, save_dir, quality_index, batch_size=16, num_workers=4,i_frame_q_scales =None):
    """
    ...
    if i_frame_q_scales is not None:
        q_scale = i_frame_q_scales[quality_index]
    ...
    with torch.no_grad():
        for batch_frames, batch_paths in tqdm(dataloader, desc=f"Compressing frames with I-frame model (Quality {quality_index})"):
            ...
            for i in range(0, reshaped_frames.size(0), sub_batch_size):
                ...
                results = i_frame_model.encode_decode(sub_batch,q_scale)
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='/scratch/gautschi/zhan5096/dataset/Vimeo90k/vimeo_septuplet/reference_sequences_DCVC_HEM', 
                        help='Base directory to save precomputed reference frames')
    parser.add_argument('--septuplet_list', type=str, default='/scratch/gautschi/zhan5096/dataset/Vimeo90k/vimeo_septuplet/sep_trainlist.txt', help='Path to septuplet list file')
    ...
    i_frame_model.eval()
    print("Loaded I-frame model q_scales:", i_frame_model.q_scale)
