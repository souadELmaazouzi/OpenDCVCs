...
class DMC(nn.Module):
    def __init__(self, win_size=11, lmbda=1.0):
        super().__init__()
        channel_mv = 128
        channel_N = 64
        channel_M = 96
        ...
        self.lowerbound_scale = LowerBound(0.11)
    ...
    @staticmethod
    def get_y_bits_probs(y, sigma, bin_size=1.0, prob_clamp=1e-6):
    ...
    @staticmethod
    def get_z_bits_probs(z, bit_estimator, prob_clamp=1e-6):
    ...
    def encode_decode(self, x, ref_frame, ref_feature, output_path=None,
        ...
        encoded = self.forward_one_frame(x, ref_frame, ref_feature, training=False, stage=4)
        result = {
            "x_hat": encoded['recon_image'],
            "feature": encoded['feature'],
            "bpp_y": encoded['bpp_y'].item(),
            "bpp_z": encoded['bpp_z'].item(),
            "bpp_mv_y": encoded['bpp_mv_y'].item(),
            "bpp_mv_z": encoded['bpp_mv_z'].item(),
    ...
    def forward_one_frame(self, x, ref_frame, ref_feature, training=True, stage=1):
        est_mv = self.optic_flow(x, ref_frame)
        mv_y = self.mv_encoder(est_mv)
        mv_z = self.mv_prior_encoder(mv_y)
        # mv_z_hat = self.quant(mv_z)
        if training: 
            mv_z_hat =  mv_z + torch.empty_like(mv_z).uniform_(-0.5, 0.5)
        else:
            mv_z_hat = self.quant(mv_z)
        mv_params = self.mv_prior_decoder(mv_z_hat)
        mv_scales_hat, mv_means_hat = mv_params.chunk(2, 1)
        ...
        # mv_y_hat = mv_y_q + mv_means_hat
        if training:
            mv_y_noise = torch.empty_like(mv_y).uniform_(-0.5, 0.5)
            mv_y_hat = mv_y + mv_y_noise
            mv_y_for_bit = mv_y_hat - mv_means_hat
        else:
            mv_y_q = self.quant(mv_y - mv_means_hat)
            mv_y_hat = mv_y_q + mv_means_hat
            mv_y_for_bit = mv_y_hat - mv_means_hat
        ...
        # y_hat = y_q + means_hat
        if training:
            y_noise = torch.empty_like(y).uniform_(-0.5, 0.5)
            y_hat = y + y_noise
            y_for_bit = y_hat - means_hat
        else:
...
class DMC_LowerBound(DMC):
    def __init__(self, win_size=11, lmbda=1.0, lowerbound_value=0.01):
        super().__init__(win_size=win_size, lmbda=lmbda)
        self.lowerbound_scale = LowerBound(lowerbound_value)
