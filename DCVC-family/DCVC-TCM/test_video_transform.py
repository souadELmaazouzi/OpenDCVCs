...
from src.models.video_net_dmc_transform import DMC
...
def run_test(video_net, i_frame_net, args_dict, device):
        ...
        for frame_idx in range(frame_num):
            ...
            if frame_idx % gop_size == 0:
                ...
                if 'bpp' in result:
                    ...
                    # Fallback for I-frame models that return total bits
                    bpps.append(result["bit"] / frame_pixel_num)
                bpps_mv_y.append(0)
                bpps_mv_z.append(0)
                bpps_y.append(0)
                bpps_z.append(0)
                ...
                bpps.append(result['bpp'])
                bpps_mv_y.append(result['bpp_mv_y'])
                bpps_mv_z.append(result['bpp_mv_z'])
                bpps_y.append(result['bpp_y'])
                bpps_z.append(result['bpp_z'])
                p_frame_number += 1
            ...
            if verbose >= 2:
                print(f"frame {frame_idx}, {frame_end_time - frame_start_time:.3f} seconds,",
                      f"bpp: {bpps[-1]:.6f}, PSNR: {psnrs[-1]:.4f}, MS-SSIM: {msssims[-1]:.4f} ")
    ...
    cur_ave_i_frame_bpp = 0
    cur_ave_i_frame_psnr = 0
    cur_ave_i_frame_msssim = 0
    cur_ave_p_frame_bpp = 0
    cur_ave_p_frame_bpp_mv_y = 0
    cur_ave_p_frame_bpp_mv_z = 0
    cur_ave_p_frame_bpp_y = 0
    cur_ave_p_frame_bpp_z = 0
    ...
    for idx in range(frame_num):
        if frame_types[idx] == 0:
            cur_ave_i_frame_bpp += bpps[idx]
            cur_ave_i_frame_psnr += psnrs[idx]
            cur_ave_i_frame_msssim += msssims[idx]
            cur_i_frame_num += 1
        else:
            cur_ave_p_frame_bpp += bpps[idx]
            cur_ave_p_frame_bpp_mv_y += bpps_mv_y[idx]
            cur_ave_p_frame_bpp_mv_z += bpps_mv_z[idx]
            cur_ave_p_frame_bpp_y += bpps_y[idx]
            cur_ave_p_frame_bpp_z += bpps_z[idx]
    ...
    log_result['ave_i_frame_bpp'] = cur_ave_i_frame_bpp / cur_i_frame_num if cur_i_frame_num > 0 else 0
    log_result['ave_i_frame_psnr'] = cur_ave_i_frame_psnr / cur_i_frame_num if cur_i_frame_num > 0 else 0
    log_result['ave_i_frame_msssim'] = cur_ave_i_frame_msssim / cur_i_frame_num if cur_i_frame_num > 0 else 0
    log_result['frame_bpp'] = bpps
    log_result['frame_bpp_mv_y'] = bpps_mv_y
    log_result['frame_bpp_mv_z'] = bpps_mv_z
    log_result['frame_bpp_y'] = bpps_y
    log_result['frame_bpp_z'] = bpps_z
    ...
    if cur_p_frame_num > 0:
        log_result['ave_p_frame_bpp'] = cur_ave_p_frame_bpp / cur_p_frame_num
        log_result['ave_p_frame_bpp_mv_y'] = cur_ave_p_frame_bpp_mv_y / cur_p_frame_num
        log_result['ave_p_frame_bpp_mv_z'] = cur_ave_p_frame_bpp_mv_z / cur_p_frame_num
        log_result['ave_p_frame_bpp_y'] = cur_ave_p_frame_bpp_y / cur_p_frame_num
        log_result['ave_p_frame_bpp_z'] = cur_ave_p_frame_bpp_z / cur_p_frame_num
    ...
    log_result['ave_all_frame_bpp'] = (cur_ave_i_frame_bpp + cur_ave_p_frame_bpp) / frame_num
...
def encode_one(args_dict, device):
    ...
    if args_dict['force_intra']:
        ...
        if "model_state_dict" in load_checkpoint:
            load_checkpoint = load_checkpoint['model_state_dict']
...
def filter_dict(result):
    ...
    sum_bpp = i_num * res['ave_i_frame_bpp'] + p_num * res['ave_p_frame_bpp']
    ...
    res['ave_all_frame_bpp'] = sum_bpp / (i_num + p_num)
