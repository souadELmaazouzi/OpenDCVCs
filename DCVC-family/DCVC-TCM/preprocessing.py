    ...
    with torch.no_grad():
        for batch_frames, batch_paths in tqdm(dataloader, desc=f"Compressing frames with I-frame model (Quality {quality_index})"):
            ...
            for i in range(0, reshaped_frames.size(0), sub_batch_size):
                ...
                results = i_frame_model.encode_decode(sub_batch)
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='/scratch/gautschi/zhan5096/dataset/Vimeo90k/vimeo_septuplet/reference_sequences_DCVC_TCM', 
    ...
    parser.add_argument('--quality_index', type=int, required=True, choices=[1, 2, 3,4], help='Quality index (1-4)')
    ...
    i_frame_model = architectures[args.i_frame_model_name].from_state_dict(
