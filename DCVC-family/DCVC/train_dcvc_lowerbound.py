...
# Import the model
from src.models.DCVC_net_lower import DCVC_net
...
def train_one_epoch_fully_batched(model, i_frame_model, train_loader, optimizer, device, stage, epoch, gradient_accumulation_steps=1, finetune=False, grad_clip_max_norm=None, ema=None):
    """
    ...
    for batch_idx, (batch_frames, batch_precomputed_frames) in enumerate(progress_bar):
        ...
        if finetune and stage == 4:
            ...
            for frame_pos in range(seq_length):
                ...
                else:  # previous recon frame as refence frames 
                    reference_frames = reconstructed_frames
                ...
                reconstructed_frames = result["recon_image"]  # Shape: [B, C, H, W]
        ...
        # clear cache
        if finetune and stage == 4:
            del batch_frames, batch_precomputed_frames, reconstructed_frames
            del batch_loss
...
def evaluate_fully_batched_three(model, i_frame_model, test_loader, device, stage):
    """
...
def main():
    ...
    parser.add_argument('--fine_tune_lr', type=float, default=4e-5, help='Learning rate for finetuning')
    ...
    if args.finetune:
        ...
        args.checkpoint_dir = args.checkpoint_dir+'_finetune_'+str(args.fine_tune_lr)
        args.log_dir = args.log_dir+'_finetune_'+str(args.fine_tune_lr)
        args.global_best_dir = args.global_best_dir+'_finetune_'+str(args.fine_tune_lr)
        ...
        try:
            ...
            if args.not_skip_test: 
                ...
                test_stats_three = evaluate_fully_batched_three(
                    model, i_frame_model, test_loader, device, args.stage-1
                )
                # Log results
                with open(log_file, 'a') as f:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_fully_batched(
            ...
            args.gradient_accumulation_steps, args.finetune, args.grad_clip_max_norm, ema
        )
