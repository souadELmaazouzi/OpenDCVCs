...
class DCVC_net(nn.Module):
    ...
    def quantize(self, inputs, mode, means=None, training=False):
        """Quantize tensor.
        
        Args:
            inputs: Input tensor
            mode: "dequantize" for forward/inference, "noise" for training
            means: Mean values for centering before quantization
        ...
        """
        if training:
            # During training, use additive uniform noise as differentiable approximation
            half = float(0.5)
            ...
            # During inference, use rounding (original behavior)
    ...
    def feature_probs_based_sigma(self, feature, mean, sigma, training=False, noise=None):
        """Calculate bits needed to encode feature with given mean and sigma.
        
        Args:
            feature: Input feature tensor
            mean: Mean tensor for centering
            sigma: Scale parameter for Laplace distribution
            training: Whether in training mode
            noise: Pre-generated noise to ensure consistent quantization
            
        Returns:
            total_bits: Estimated bit rate
            probs: Probability mass function values
        """
        if training:
            if noise is None:
                # Only generate new noise if not provided
                half = float(0.5)
                noise = torch.empty_like(feature).uniform_(-half, half)
            # Use the provided or newly generated noise
            values = feature + noise - mean
        else:
            # During inference, use rounding (original behavior)
            outputs = self.quantize(feature, "dequantize", mean, training=False)
            values = outputs - mean
    ...
    def forward(self, referframe, input_image, training=True, stage=1):
        ...
        # Generate noise for z_mv quantization
        if training:
            z_mv_noise = torch.empty_like(z_mv).uniform_(-0.5, 0.5)
            compressed_z_mv = z_mv + z_mv_noise
        else:
            compressed_z_mv = torch.round(z_mv)
            
        params_mv = self.mvpriorDecoder(compressed_z_mv)

        # Generate noise for mvfeature quantization
        if training:
            mvfeature_noise = torch.empty_like(mvfeature).uniform_(-0.5, 0.5)
            quant_mv = mvfeature + mvfeature_noise
        else:
        ...
        # Generate noise for z quantization
        if training:
            z_noise = torch.empty_like(z).uniform_(-0.5, 0.5)
        ...
        # Generate noise for feature quantization
        if training:
            feature_noise = torch.empty_like(feature_renorm).uniform_(-0.5, 0.5)
            compressed_y_renorm = feature_renorm + feature_noise
        else:
        ...
        # Use bit rate estimation methods with consistent noise
        total_bits_y, _ = self.feature_probs_based_sigma(
            feature_renorm, means_hat, scales_hat, training=training, 
            noise=feature_noise if training else None)
        total_bits_mv, _ = self.feature_probs_based_sigma(
            mvfeature, means_hat_mv, scales_hat_mv, training=training,
            noise=mvfeature_noise if training else None)
        ...
        # Keep your loss calculation stages
        loss = 0
        
        # Loss calculation based on training stage
        if stage == 1:
            # Stage 1: L_me = lambda*distortion(rec,inp) + bpp_mv_y + bpp_mv_z
            ...
            # Stage 2: L_rec = lambda*distortion(rec,inp)
            mse_loss = self.mse(recon_image, input_image)
            L_rec = self.lmbda * mse_loss
            bpp_train = torch.tensor(0)
            loss = L_rec
        elif stage == 3:
            # Stage 3: L_con = lambda*distortion(rec,inp) + bpp_y + bpp_z
            mse_loss = self.mse(recon_image, input_image)
            distortion = self.lmbda * mse_loss
            ...
            # Stage 4: L_all = lambda*distortion(rec,inp) + bpp
            mse_loss = self.mse(recon_image, input_image)
            distortion = self.lmbda * mse_loss
