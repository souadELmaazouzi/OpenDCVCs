...
class DCVC_net(nn.Module):
    ...
    def forward(self, referframe, input_image, training=True, stage=1):
        ...
        # Replace softplus and exp with LowerBound
        # Original: scales_hat_mv = torch.nn.functional.softplus(scales_hat_mv+2.3)-2.3
        # Original: scales_hat_mv = torch.exp(scales_hat_mv)
        scales_hat_mv = self.lowerbound_scale(scales_hat_mv)
        ...
        # Replace softplus and exp with LowerBound
        # Original: scales_hat = torch.nn.functional.softplus(scales_hat+2.3)-2.3
        # Original: scales_hat = torch.exp(scales_hat)
        ...
        total_bits_mv, _ = self.feature_probs_based_sigma(
            mvfeature, means_hat_mv, scales_hat_mv, training=training, noise=quant_mv_noise if training else None)
